...
void semaphore_wait(sem_t *sem) { // Блокировка процесса
    sem_wait(sem); // Блокируем семафор
}

void semaphore_signal(sem_t *sem) { // Разблокировка процесса
    sem_post(sem); // Разблокируем семафор
}
...
int main(int argc, char *argv[]) {
    ...
    semaphore = sem_open("/sem_ex", O_CREAT, S_IRUSR | S_IWUSR, 1);
    ...
    if (pid < 0) {
        ...
        for (int i = 0; i < num_repeats + 1; i++) {
            semaphore_wait(semaphore); // Блокируем доступ к файлу для других
            ...
            semaphore_signal(semaphore); // Освобождаем доступ к файлу
            sleep(1);
        }
        ...
        for (int i = 0; i < num_repeats; i++) { // Ограничение на количество повторов
            ...
            semaphore_wait(semaphore); // Запрашиваем доступ к файлу
